...
$listname=$_GET['list'];
$db = PDODB::getCMS_DB();
$sql="select rs.*,s2.college_id from t_ranking_schools rs left join(select * from (select s.*,row_number() over(partition by s.s_name_cn order by s.college_id) r from t_schools s ) where r=1) s2 on rs.school_name_cn=s2.s_name_cn where rs.ranking_type_name='$listname' order by rs.myorder asc";
// echo $sql;exit;
$stmt = $db->prepare($sql);
$stmt->execute( );
    ...
    <div class="University_rank">
      ...
      <script>
        ...
        $('#searchrank').click(function() {
          ...
          var url="/php/ajax.php?action=searchInRank"
          var d={key:key,ranktype:ranktype};
